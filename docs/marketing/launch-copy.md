...
| `[P-RECAP]`      | Title-only und exact-body Backend-, Gateway- und Mobile-Client-Verträge, account-/root-isolierter SQLite-Cache sowie bounded 90-Tage-Retention für exact-body Metadaten sind implementiert und repository-getestet. Die Design-2-Fläche besitzt exakte Body-Vorschau/Auswahl, servereigene Autor-/Manager-Entscheide und serverseitig fail-closed Share/Widerruf. `crew-paq.2.15.4.3.2`, `.4` und `.5` sind geschlossen: Native konsumiert das reloadbare, ID-freie `externalConsent`-Read-Model ordinalgebunden, persistiert die Entscheide nicht in SQLite und leitet Aktionen nur aus `actorCanDecide` ab. Fokus- und vollständige Mobile-Data-Gates sowie eine isolierte iOS-Release-shaped Design-2-Matrix belegen Grant, Withdrawal, Autor:in, Manager, Participant:in, Viewer und Drift; die Matrix nutzt Fixtures und ist kein deployter Backend-E2E. Participant-Sharing bleibt verboten; Caption/Media bleibt gated, Provenance bleibt vertragsgemäss verboten. Privacy-/Legal-Approval, Deployment, öffentlicher Consumer, real service-backed Rollen-/Removal-/Recovery-E2E und Android-Parität fehlen. Keine Acquisition-Copy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
title-only oder exact-body Link nur für Owner oder Organizer nach bestätigter
...
Design-2-Fläche konsumiert das servereigene, reloadbare, ID-freie
`externalConsent`-Read-Model fail-closed und zeigt Autor-/Manager-Aktionen nur
aus `actorCanDecide`; eine isolierte iOS-Release-shaped Fixture-Matrix belegt
...
im Vertrag nicht erlaubt. Exact-version Event-/Feed-Bodies sind im Backend nur
mit den erforderlichen Feld- und Autoritätsgrants test-current; Caption und
Medien bleiben gated, Provenance bleibt vertragsgemäss verboten.
