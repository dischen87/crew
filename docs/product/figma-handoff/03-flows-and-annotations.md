...
After a first-party zero result it also exposes the current bounded
`search_miss` flow: explicit country, cited-field review, idempotent
owner/organizer approve or reject, and the existing event-place/capability bind
only after approval. It does not make the generalized `SCR-011` or `SCR-012`
editor contracts current, and it is not evidence of a provider-enabled live
worker or production execution.
...
| `EventSetupRecovery` | narrow recovery in `SCR-013`, including zero-result cited worldwide review |
| `Plan` | `SCR-010` |
...
| `SCR-010` | Plan tree and timeline      | `COPY-SCR-010-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`, `VISUAL-EVIDENCE-IOS`; recursive cached plan, subtree filtering, role-correct actions, child-event/itinerary reorder, durable optimistic rows, and explicit dead-letter discard are code/test current. The native packet at `apps/mobile/evidence/plan-editor-live-2026-07-27` retains historical iOS/Android Plan evidence with its original provenance and an accepted current-source iOS queued/conflict/tombstone Plan matrix at medium/Accessibility Extra Large, hash-bound in `runtime/oracles/ios-current-state-matrix.json`. Uncaptured states, signed physical devices, VoiceOver/TalkBack, production, and external Figma stay `DESIGN-REQUIRED`. |
| `SCR-011` | Plan item editor            | `COPY-SCR-011-*` | itinerary-item create/update, child-event editing, child-event reorder, and itinerary reorder are `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; named cached place selection and conflict retry are current. The native packet at `apps/mobile/evidence/plan-editor-live-2026-07-27` retains historical editor/action-area layouts and accepted current-source iOS queued/conflict/tombstone Editor evidence at medium/Accessibility Extra Large. The Default-size conflict Editor provenance exception is recorded in its oracle. Generalized place creation, uncaptured states, signed physical devices, VoiceOver/TalkBack, production, and external Figma stay `DESIGN-REQUIRED`. |
| `SCR-012` | Capability setup            | `COPY-SCR-012-*` | `DESIGN-REQUIRED`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `SCR-013` | Readiness review            | `COPY-SCR-013-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; basics plus narrow template/capability/place recovery are code/test current, including zero-result `search_miss`, bounded polling, cited review, explicit approve/reject, and bind only after approval. The [closed local Turkey journey](../../../apps/mobile/evidence/turkey-golf-vertical-closure/README.md) proves service-backed publication, not this provider path. Fresh native screenshots/visual parity, signed physical devices, VoiceOver/TalkBack, a provider-enabled live worker, production, external Figma, and remaining states stay `DESIGN-REQUIRED`.                                                                           |
| `SCR-014` | Live item detail            | `COPY-SCR-014-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`, `VISUAL-EVIDENCE-IOS`; exact optimistic/offline item, role-safe one-primary-action resolution, adjacent-item navigation, removed-target return to the updated plan, and sync truth are code/test current. The native packet at `apps/mobile/evidence/plan-editor-live-2026-07-27` retains historical Live Item/action-area layouts and accepted current-source iOS queued/conflict/tombstone Live evidence at medium/Accessibility Extra Large, including the removed-target gate where applicable. Uncaptured states, signed physical devices, VoiceOver/TalkBack, production, and external Figma stay `DESIGN-REQUIRED`. |
| `SCR-020` | Invite manager              | `COPY-SCR-020-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; owner/organizer concealment, token-free cache, pagination, revoke retry, and offline states are code/test current. The 7/7 sanitized native runs retained under `apps/mobile/evidence/native-invite-manager-2026-07-27` cover iOS owner create/refresh and Large Text plus the Android organizer manager at font scale 2.0. Native Zurich DST-overlap round-trip is NO-GO with the current nondeterministic fold picker harness, and iOS Share Sheet dismissal is not final. Physical distribution-signed devices, VoiceOver/TalkBack, and external Figma remain pending. |
| `SCR-021` | Invite editor               | `COPY-SCR-021-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; role policy, token-free local draft, stable create replay, once-only in-memory sharing, and the system-native iOS/Android expiry picker are current. The same 7/7 simulator evidence covers role narrowing, picker cancel/confirm, create, iOS Large Text, and Android font scale 2.0. Native Zurich DST-overlap round-trip is NO-GO with the current nondeterministic fold picker harness, and iOS Share Sheet dismissal is not final. Physical distribution-signed devices, VoiceOver/TalkBack, and external Figma remain pending. |
...
| `SCR-031` | Golf scorecard              | `COPY-SCR-031-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; queued, conflict, read-only and Accessibility Large `VISUAL-EVIDENCE-IOS`. Closed `crew-paq.7.4` adds retained iOS/Android offline replay and service-backed convergence; Android same-state visual parity, signed physical devices, and remaining state/platform variants stay `DESIGN-REQUIRED`.                                                                                                                                                                                                                                                                                                                                                 |
| `SCR-032` | Team collaboration setup    | `COPY-SCR-032-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`, organizer `VISUAL-EVIDENCE-IOS`; discoverable Crew-tab entry and the closed local Team vertical are current. Generic conflict resolution, signed physical devices, and remaining state/platform variants stay `DESIGN-REQUIRED`.                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `SCR-033` | Team decision               | `COPY-SCR-033-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`, closed-attention participant `VISUAL-EVIDENCE-IOS`; discoverable Crew-tab entry is current, and the `team-vertical-session-2026-07-27-final` packet retains Android participant route execution. Generic conflict resolution, Android same-state visual parity, signed physical devices, and remaining states stay `DESIGN-REQUIRED`.                                                                                                                                                                                                                                                                                                                        |
| `SCR-040` | Event feed                  | `COPY-SCR-040-*` | root/event text-feed, attachment display, authored revision/conflict, and exact inbound target slices are `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; the `team-vertical-session-2026-07-27-final` packet proves Android offline/relaunch/response-loss replay/convergence plus iOS readback for the text-feed path. Reaction/remove/moderation UI, generic conflict resolution, photo Device-E2E, and remaining variants stay `DESIGN-REQUIRED`.                                                                                                                                                                                                                                                        |
| `SCR-041` | Feed composer               | `COPY-SCR-041-*` | inline root/event text/single-photo create and authored-text edit slices are `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; offline pick/preview, caption/decorative choice, restart, upload/finalize retry, missing-file reselection, exact entry focus, and revision conflict are code/test current. The `team-vertical-session-2026-07-27-final` packet proves the Android text-submit/replay path only; separate full-screen composer, photo Device-E2E, signed physical devices, and remaining variants stay `DESIGN-REQUIRED`.                                                                                                                                                                        |
...
| `SCR-060` | Feedback compose            | `COPY-SCR-060-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; consented screenshot and duplicate suggestions are current. The native packet at `apps/mobile/evidence/feedback-team-native-2026-07-27` proves iOS-simulator and Android-emulator capture, consent-off preview, delivery, cold-restart server-backed list readback, and logout purge; it also retains an iOS storage-denial to text-only recovery. Distribution-signed physical devices, VoiceOver/TalkBack, production, and external Figma remain pending.                                                                                                                                                                                        |
| `SCR-061` | Feedback item               | `COPY-SCR-061-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; vote/comment/follow plus owner/organizer status and sanitized same-root merge triage are code/test current and online-only. Manager writes recheck role/root/target, keep stable idempotency, and durably block every write after a confirmed commit until a sanitized authoritative refresh succeeds; direct root denial is concealed and failure copy never exposes private error detail. The native packet at `apps/mobile/evidence/feedback-team-native-2026-07-27` does not exercise this screen, so fresh native item screenshots/visual parity, signed physical devices, VoiceOver/TalkBack, production, and external Figma remain pending. |
| `SCR-062` | Event feedback              | `COPY-SCR-062-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; account/root cached reports and updates are current. The native packet at `apps/mobile/evidence/feedback-team-native-2026-07-27` proves cold-restart server-backed list rendering on both the iOS simulator and Android emulator. Same-input visual comparison, distribution-signed physical devices, VoiceOver/TalkBack, production, and external Figma remain pending.                                                                                                                                                                                                                                                                           |
...
| Feedback               | `F-01` through `F-03`     | current Hub -> `SCR-062 -> SCR-060/061`; selected duplicate -> `SCR-061`, with separate screenshot/diagnostics consent and owner/organizer status/same-root merge triage |
...
- A zero-result `search_miss` is not an event place. Annotate the country and
  cited fields, then the explicit manager decision: rejection creates/binds
  nothing; approval may continue through the existing event-place/capability
  bind. Provider prompts, secrets, and uncited fields stay absent.
