...
| `SCR-010` | Plan tree and timeline      | `COPY-SCR-010-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`, `VISUAL-EVIDENCE-IOS`; recursive cached plan, subtree filtering, role-correct actions, child-event/itinerary reorder, durable optimistic rows, and explicit dead-letter discard are code/test current. The [final local Team packet](../../../apps/mobile/evidence/team-vertical-session-2026-07-27-final/README.md) retains the iOS owner Plan execution capture; Android same-state visual parity, signed physical devices, and remaining states stay `DESIGN-REQUIRED`.                                                                                                                       |
| `SCR-011` | Plan item editor            | `COPY-SCR-011-*` | itinerary-item create/update, child-event editing, child-event reorder, and itinerary reorder are `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; named cached place selection and conflict retry are current. Generalized place creation, signed physical devices, external Figma, and remaining state/platform coverage stay `DESIGN-REQUIRED`.                                                                                                                                                                                                                                                                                                |
...
| `SCR-060` | Feedback compose            | `COPY-SCR-060-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; consented screenshot and duplicate suggestions are current. The final local Team/Feedback packet proves Android capture, consent-off preview, durable pending/delivery, committed upload, and cold-restart server readback; iOS, signed physical devices, VoiceOver/TalkBack, distribution, and production acceptance remain pending.                                                                                                                                                                                                                                                            |
| `SCR-061` | Feedback item               | `COPY-SCR-061-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; vote/comment/follow plus owner/organizer status and sanitized same-root merge triage are code/test current and online-only. Manager writes recheck role/root/target, keep stable idempotency, and durably block every write after a confirmed commit until a sanitized authoritative refresh succeeds; direct root denial is concealed and failure copy never exposes private error detail. The final packet does not exercise this screen, so fresh native screenshots/visual parity, signed physical devices, VoiceOver/TalkBack, distribution, production, and external Figma remain pending. |
| `SCR-062` | Event feedback              | `COPY-SCR-062-*` | `NATIVE-CURRENT-ROUTED`, `DATA-CURRENT`; account/root cached reports and updates are current. The final packet proves Android cold-restart list rendering of the server-created item; iOS, same-state Android visual parity, signed physical devices, VoiceOver/TalkBack, distribution, and production acceptance remain pending.                                                                                                                                                                                                                                                                                                         |
