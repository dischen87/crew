...
try {
  ...
  if (
    ...
    result.eventVersion !== 4 ||
    result.rootRevision !== 6 ||
    result.publishedEventChanges !== 1 ||
    result.publishedFeedChanges !== 1 ||
    result.completedPublishRecords !== 1
  ) {
    throw new Error('Single-publish evidence mismatch');
  }
  process.stdout.write(`${JSON.stringify({ rootEventId, ...result }, null, 2)}\n`);
} finally {
  await sql.end();
}
