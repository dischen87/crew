...
const proofLink = 'crewnext://evidence/release-crypto';
...
const releaseEvidenceLinking = {
  ...
  async getInitialURL() {
    try {
      ...
      if (inboundUrl === proofLink) return null;
      if (inboundUrl) {
  ...
  subscribe(listener) {
    const subscription = Linking.addEventListener('url', event => {
      if (event.url === proofLink) {
        showReleaseCryptoProof();
        return;
      }
...
function GolfScorecardIosReleaseEvidenceApp() {
  ...
  useEffect(() => {
    ...
    Linking.getInitialURL().then(
      url => {
        if (url === proofLink) showReleaseCryptoProof();
      },
      () => undefined,
    );
...
function showReleaseCryptoProof() {
  Alert.alert(
    'Sanitized Release crypto proof',
    JSON.stringify(releaseCryptoShape),
  );
}
...
AppRegistry.registerComponent(
  'CrewNext',
  () => GolfScorecardIosReleaseEvidenceApp,
);
