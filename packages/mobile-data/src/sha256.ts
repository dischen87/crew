...
export async function sha256Hex(value: string): Promise<string> {
	...
	if (subtle) {
		try {
			...
			// React Native runtimes can expose Web Crypto without implementing digest.
		}
	}
	return sha256HexFallback(bytes);
}

export function sha256HexFallback(value: string | Uint8Array): string {
...
function utf8(value: string): Uint8Array<ArrayBuffer> {
	...
	for (const character of value) {
		const codePoint = scalar(character);
		length +=
			codePoint < 0x80
				? 1
				: codePoint < 0x800
...
function scalar(character: string): number {
